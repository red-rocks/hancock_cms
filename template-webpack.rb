...
# TEXT
# end
# gsub_file 'Gemfile', /^ruby '\d\.\d(\.\d)?'/, "ruby '#{RVM_RUBY_VERSION}'"
inject_into_file 'Gemfile', after: 'git_source(:github) { |repo| "https://github.com/#{repo}.git" }' do <<-TEXT
...
# append_to_file 'Gemfile' do <<-TEXT
...
# run "rvm gemset list"
run "rvm use #{RUBY_VERSION}@#{app_name.underscore} --create"
run "rvm #{RUBY_VERSION} do rvm gemset create #{app_name.underscore}"
run "rvm #{RUBY_VERSION}@#{app_name.underscore} do gem install bundler"
run "rvm #{RUBY_VERSION}@#{app_name.underscore} do bundle install --without production"
...
# rails new svelte_test --skip-action-mailbox --skip-active-record --skip-active-storage --skip-sprockets --skip-turbolinks --skip-test --skip-system-test --no-api -B
# rails new svelte_test --skip-action-mailbox --skip-active-record --skip-active-storage --skip-sprockets --skip-test --skip-system-test --no-api -B
...
# TEXT
# end
inject_into_file 'config/application.rb', before: /^.*# Don't generate system test files./ do <<-TEXT
    config.i18n.locale = :ru
    config.i18n.default_locale = :ru
    config.i18n.available_locales = [:ru, :en]
    config.i18n.enforce_available_locales = true
